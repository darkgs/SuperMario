...
class SplitAttnPDQN(object):
	...
	def build_q_network(self, name):
		...
		with tf.variable_scope(name):
			...
			step = tf.layers.batch_normalization(step)
			step_down = tf.layers.batch_normalization(step_down)
			step_center = tf.layers.batch_normalization(step_center)
			...
			step = tf.nn.dropout(step, keep_prob=0.5)
